...
class SitemapgeneratorAPIClientError(Exception):
...
class ValidationError(SitemapgeneratorAPIClientError):
...
class SitemapgeneratorAPIClient:
    # Validation rules for parameters (generated from schema)
    VALIDATION_RULES = {"urls": {"type": "array", "required": True, "format": "url"}, "baseUrl": {"type": "string", "required": False, "format": "url"}, "changeFreq": {"type": "string", "required": False, "default": "weekly"}, "priority": {"type": "number", "required": False, "min": 0, "max": 1, "default": 0.5}, "includeLastMod": {"type": "boolean", "required": False, "default": True}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the SitemapgeneratorAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises SitemapgeneratorAPIClientError: If API key is invalid
        """
        ...
        self.secure = secure
        self.debug = debug
        self.base_url = 'https://api.apiverve.com/v1/sitemapgenerator'
        self.headers = {
    ...
    def _validate_api_key(self, api_key: str) -> None:
        ...
        if not api_key or not api_key.strip():
            raise SitemapgeneratorAPIClientError(
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[SitemapgeneratorAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns SitemapGeneratorData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises SitemapgeneratorAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise SitemapgeneratorAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise SitemapgeneratorAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise SitemapgeneratorAPIClientError(f"Request failed: {str(e)}")
